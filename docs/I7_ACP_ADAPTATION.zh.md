...
| 顺序 | 内容 | 跳转 |
|------|------|------|
| **1** | 结论与能力对照（能不能做） | [→ §1](#1-结论先说能不能做) |
| **2** | 推荐主线（先做什么） | [→ §2](#2-推荐主线一句话) |
| **2b** | **MuJoCo 联调仿真 MVP（当前优先，绕开真机限制）** | [`MUJOCO_ACP_SIM_MVP.zh.md`](MUJOCO_ACP_SIM_MVP.zh.md) |
| **3** | 控制栈分层（往哪接） | [→ §3](#3-控制栈分层往哪接) |
| **4** | 硬件差异与应对 | [→ §4](#4-ur5e-vs-i7-差异与应对) |
| **5** | 三层落地步骤（训练 → 导纳 → 闭环） | [→ §5](#5-三层落地推荐顺序) |
| **6** | 变刚度三种方案（选哪条） | [→ §6](#6-变刚度三种落地方案) |
| **7** | 桥接接口蓝图（怎么读写） | [→ §7](#7-桥接接口蓝图-acp_i7_bridgepy) |
...
### 11a. 仿真优先（绕开真机限制）→ 详见 [`MUJOCO_ACP_SIM_MVP.zh.md`](MUJOCO_ACP_SIM_MVP.zh.md)
