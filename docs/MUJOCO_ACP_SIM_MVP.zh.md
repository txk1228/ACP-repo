...
| 顺序 | 内容 | 跳转 |
|------|------|------|
| **1** | 目标与成功标准 | [→ §1](#1-目标与成功标准) |
| **2** | MVP 范围 | [→ §2](#2-mvp-范围已定案-b) |
| **3** | 架构与接口 | [→ §3](#3-架构与接口) |
| **4** | 递进版本（v1→v2-ft） | [→ §4](#4-递进版本v1v2-ft) |
| **5** | 分阶段里程碑 | [→ §5](#5-分阶段里程碑) |
| **6** | 目录与依赖 | [→ §6](#6-目录与依赖) |
| **7** | 风险与验收 | [→ §7](#7-风险与验收清单) |
...
| 产出 | 说明 |
|------|------|
| **单臂翻方块可演示** | v1 脚本专家；v2-ft 策略同场景翻起 |
| **ACP 柔顺可验收** | 方案 A 力调度；`run_acp_effect_demo` |
| **策略在环联调** | Flip Spec 三模态 → 开环 action horizon |
...
> **递进验收，非并列方案**：v1 验证场景可翻；v2 验证三模态链路（域差下不稳翻）；v2-ft 经仿真微调后策略同域可翻。

| 版本 | 控制 | RGB | 权重 | 验收 |
|------|------|-----|------|------|
| **v1** | 固定轨迹脚本 | 无 | 无策略 | tilt ≥55°（场景可行性） |
| **v2** | Flip Spec 策略 | 腕部真渲染 | Spec ckpt（零样本） | 推理通 + 有位移（链路） |
| **v2-ft** | Flip Spec 策略 | 腕部真渲染 | **仿真微调 ckpt** | tilt ≥55° + 真 RGB |
