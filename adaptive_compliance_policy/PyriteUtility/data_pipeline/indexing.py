...
def get_sample_ids(query, horizon, down_sample_steps, backwards=False, closed=False):
    ...
    # print("[get_sample_ids] debug: query: ", query)
    # print("[get_sample_ids] debug: local_id: ", local_id)
    if not backwards:
...
def get_samples(raw_data, query, horizon, down_sample_steps, backwards=False, closed=False):
    """
    Get the sample ids for the down-sampling of the data.
    ...
    """
    return raw_data[get_sample_ids(query, horizon, down_sample_steps, backwards, closed)]


def get_dense_query_points_in_horizon(sparse_action_horizon,
    ...
    sparse_raw_steps = sparse_action_horizon * sparse_action_down_sample_steps
...
def test():
    ...
    s = get_samples(raw_data, query, horizon, down_sample_steps, backwards=True, closed=False)
    ...
    delta_steps = 3
    queries = get_dense_query_points_in_horizon(sparse_action_horizon,
                                                sparse_action_down_sample_steps,
                                                dense_action_horizon,
                                                dense_action_down_sample_steps,
                                                delta_steps)
